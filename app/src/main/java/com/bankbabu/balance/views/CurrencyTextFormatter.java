  ...
  public static String formatText(String val, Locale locale, Locale defaultLocale,
    ...
    if (decimalDigits != null) {
      ...
      try {
        ...
        Log.e("CurrencyTextFormatter", "Illegal argument detected for currency: " + currency
            + ", using currency from defaultLocale: " + defaultLocale);
    ...
    try {
      ...
      try {
        Log.e("CurrencyTextFormatter",
            "Error detected for locale: " + locale + ", falling back to default value: "
                + defaultLocale);
    ...
    if (!val.equals("")) {
      ...
      String preparedVal = new StringBuilder(val).insert(val.length() - currencyDecimalDigits, '.')
          .toString();
