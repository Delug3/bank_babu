...
public class CurrencyEditText extends android.support.v7.widget.AppCompatEditText {
  ...
  private Locale retrieveLocale() {
    ...
    try {
      ...
      Log.w("CurrencyEditText", String.format(
  ...
  /**
   * Retrieve the raw value that was input by the user in their currencies lowest denomination (e.g.
   * pennies).
   *
   * IMPORTANT: Remember that the location of the decimal varies by currentCurrency/Locale. This
   * method returns the raw given value, and does not account for locality of the user. It is up to
   * the calling application to handle that level of conversion. For example, if the text of the
   * field is $13.37, this method will return a long with a value of 1337, as penny is the lowest
   * denomination for USD. It will be up to the calling application to know that it needs to handle
   * this value as pennies and not some other denomination.
   *
   ...
   *
   * IMPORTANT - This method does NOT update the currently set Currency object used by this
   * CurrencyEditText instance. If your use case dictates that Currency and Locale should never
   * break from their default pairing, use 'configureViewForLocale(locale)' instead of this method.
   *
  ...
  /**
   * Returns the number of decimal digits this CurrencyEditText instance is currently configured to
   * use. This value will be based on the current Currency object unless the value was overwritten
   * by setDecimalDigits().
   */
  ...
  /**
   * Sets the number of decimal digits the currencyTextFormatter will use, overriding the number of
   * digits specified by the current currency. Note, however, that calls to setCurrency() and
   * configureViewForLocale() will override this value.
   *
   ...
   * @throws IllegalArgumentException If provided value does not fall within the range (0, 340)
   * inclusive.
   */
  ...
  /**
   * Sets up the CurrencyEditText view to be configured for a given locale, using that locales
   * default currency (so long as the locale is ISO-3166 compliant). If there is an issue retrieving
   * the locales currency, the defaultLocale field will be used.
   *
   * This is the most 'fool proof' way of configuring a CurrencyEditText view when not relying on
   * the default implementation, and is the recommended approach for handling locale/currency setup
   * if you choose not to rely on the default behavior.
   *
   * Note that this method will set the decimalDigits field, potentially overriding values from
   * previous setDecimalDigits calls.
   */
  ...
  /**
   * Override the locale to be used in the event that the users device locale is not ISO 3166
   * compliant. Defaults to Locale.US. NOTE: Be absolutely sure that this value is supported by ISO
   * 3166. See Java.util.Locale.getISOCountries() for a list of currently supported ISO 3166 locales
   * (note that this list may not be identical on all devices)
   *
   * @param locale The fallback locale used to recover gracefully in the event of the current locale
   * value failing.
   */
   ...
   *
   * @param val A string which represents the value you'd like formatted. It is expected that this
   ...
   * string, and will first strip any non-digit characters before working on that string. If the
   * result of that processing reveals an empty string, or a string whose number of digits is
   * greater than the max number of digits, an exception will be thrown.
   ...
   * value will be in the same format returned by the getRawValue() method (i.e. a series of digits,
   * such as "1000" to represent "$10.00").
   * @return A deviceLocale-formatted string of the passed in value, represented as currentCurrency.
   */
